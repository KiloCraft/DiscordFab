...
public class CommandSpyBroadcaster implements EventHandler<PlayerOnChatMessageEvent> {
    ...
    public void handle(@NotNull PlayerOnChatMessageEvent event) {
        ...
        WebhookClientBuilder clientBuilder = new WebhookClientBuilder(DiscordFab.getInstance().getDataConfig().getProperty("commandSpyBroadcaster"));
        clientBuilder.setThreadFactory((job) -> {
            Thread thread = new Thread(job);
            thread.setName("CommandSpyThread");
